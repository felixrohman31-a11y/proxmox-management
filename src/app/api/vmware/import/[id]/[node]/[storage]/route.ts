import { NextRequest, NextResponse } from 'next/server';
import { getSessionFromCookies, canWrite } from '@/lib/session';
import { getPveClient, PveError } from '@/lib/pve';
import { appendAudit } from '@/lib/audit';

type Ctx = { params: { id: string; node: string; storage: string } };

const MAX_UPLOAD = 1024 * 1024 * 1024;

export async function POST(req: NextRequest, ctx: Ctx) {
  const session = getSessionFromCookies();
  if (!session) {
    return NextResponse.json({ error: 'Tidak terautentikasi.' }, { status: 401 });
  }
  if (!canWrite(session)) {
    return NextResponse.json({ error: 'Akses ditolak. Peran read-only.' }, { status: 403 });
  }
  const client = getPveClient(ctx.params.id);
  if (!client) {
    return NextResponse.json({ error: 'Cluster tidak ditemukan.' }, { status: 404 });
  }

  let form: FormData;
  try {
    form = await req.formData();
  } catch {
    return NextResponse.json({ error: 'Form tidak valid.' }, { status: 400 });
  }
  const file = form.get('file') as
    | { name?: string; size?: number; arrayBuffer?: () => Promise<ArrayBuffer> }
    | null;
  if (!file || typeof file.arrayBuffer !== 'function' || typeof file.name !== 'string' || !file.name) {
    return NextResponse.json({ error: 'File tidak ditemukan.' }, { status: 400 });
  }
  const ext = file.name.split('.').pop()?.toLowerCase() ?? '';
  if (!['ova', 'ovf', 'vmdk'].includes(ext)) {
    return NextResponse.json(
      { error: 'Ekstensi tidak didukung. Gunakan .ova, .ovf, atau .vmdk.' },
      { status: 400 }
    );
  }
  if (file.size && file.size > MAX_UPLOAD) {
    return NextResponse.json(
      { error: `Ukuran maksimal 1 GB (file Anda ${(file.size / 1024 ** 2).toFixed(0)} MB). Untuk disk besar gunakan SCP ke host.` },
      { status: 413 }
    );
  }

  try {
    const buffer = Buffer.from(await file.arrayBuffer());
    await client.uploadFile(ctx.params.node, ctx.params.storage, file.name, 'iso', buffer);
    await appendAudit({
      ts: new Date().toISOString(),
      user: session.u,
      action: 'vmware.import',
      target: `${ctx.params.storage}:iso/${file.name}`,
      detail: `${(buffer.length / 1024 ** 2).toFixed(1)} MB → ${ctx.params.node} (${ext})`
    });
    return NextResponse.json({
      ok: true,
      message: `${file.name} (${(buffer.length / 1024 ** 2).toFixed(1)} MB) tersimpan di ${ctx.params.storage} node ${ctx.params.node}. Konversi qcow2 & pembuatan VM bisa lanjut dari host.`,
      volid: `${ctx.params.storage}:iso/${file.name}`
    });
  } catch (e) {
    const status = e instanceof PveError ? (e.status >= 400 ? e.status : 502) : 502;
    return NextResponse.json({ error: (e as Error).message }, { status });
  }
}
